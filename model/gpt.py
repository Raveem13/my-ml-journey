...
class GPT(nn.Module):
    ...
    def __init__(self, vocab_size: int, context_length: int, model_dim: int, num_blocks: int, num_heads: int):
        ...
        for n in range(num_blocks):
            self.transformer_blocks.add_module(f"tf_block{n}", self.TransformerBlock(model_dim, num_heads))
    ...
    def forward(self, context: TensorType[int]) -> TensorType[float]:
        ...
        embedded = self.word_embeds(context) 
        positions = torch.arange(context.shape[1])
        embedded = embedded + self.pos_embeds(positions)
