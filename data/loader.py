...
class Solution:
    def create_batches(self, data: TensorType[int], context_length: int, batch_size: int) -> Tuple[TensorType[int], TensorType[int]]:
        ...
        torch.manual_seed(0)
        start_indies = torch.randint(len(data)-context_length, (batch_size,))
        print(start_indies)
        x = torch.stack([data[i : i + context_length] for i in start_indies])
        y = torch.stack([data[i + 1 : i + context_length + 1] for i in start_indies])
        return (x, y)
